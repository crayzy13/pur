...
class SecondInfoBar(Screen):
    ...
    def __init__(self, session):
        ...
        self['key_blue'] = Label()
        self['SecondInfoBar'] = ActionMap(['2ndInfobarActions'], {'pageUp': self.pageUp,
         ...
         'openSimilarList': self.openSimilarList}, -1)
        self.__event_tracker = ServiceEventTracker(screen=self, eventmap={iPlayableService.evUpdatedEventInfo: self.getEvent})
...
class InfoBarShowHide(InfoBarScreenSaver):
    ...
    def __init__(self):
        self['ShowHideActions'] = ActionMap(['InfobarShowHideActions'], {'LongOKPressed': self.toggleShowLong,
...
class InfoBarEPG():
    ...
    def __init__(self):
        ...
        self['EPGActions'] = HelpableActionMap(self, 'InfobarEPGActions', {'RedPressed': (self.RedPressed, _('Show epg')),
    ...
    def InfoPressed(self):
        if isStandardInfoBar(self) or isMoviePlayerInfoBar(self):
            if config.plisettings.PLIINFO_mode.value == 'eventview':
                ...
                if self.isInfo:
                    self.showCoolTVGuide()
            elif config.plisettings.PLIINFO_mode.value != 'infobar':
                self.showDefaultEPG()
...
class InfoBarSeek():
    ...
    def __init__(self, actionmap = 'InfobarSeekActions'):
        ...
        self['SeekActions'] = InfoBarSeekActionMap(self, actionmap, {'playpauseService': self.playpauseService,
...
class InfoBarExtensions():
    ...
    def __init__(self):
        ...
        if config.plisettings.ColouredButtons.value:
            ...
            self['InstantExtensionsActions'] = HelpableActionMap(self, 'InfobarExtensions', {'extensions': (self.showExtensionSelection, _('view extensions...')),
             'showPluginBrowser': (self.showPluginBrowser, _('Show the plugin browser..')),
             'showDreamPlex': (self.showDreamPlex, _('Show the DreamPlex player...'))}, 1)
...
class InfoBarAudioSelection():
    ...
    def yellow_key(self):
        ...
        elif config.plugins.infopanel_yellowkey.listLong.value == '0':
            from Screens.AudioSelection import AudioSelection
            self.session.openWithCallback(self.audioSelected, AudioSelection, infobar=self)
...
class InfoBarHdmi():
    ...
    def CAMrestart(self):
        ...
        DreamCCAuto()
        self.session.open(MessageBox, '  Active CAM has just been restarted.', MessageBox.TYPE_INFO, timeout=5, close_on_any_key=True)
    ...
    def MP2(self):
        try:
            ...
            self.session.open(MessageBox, '  Media Player 2 is NOT installed.', MessageBox.TYPE_INFO, timeout=5, close_on_any_key=True)
